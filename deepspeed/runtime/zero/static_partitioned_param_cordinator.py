...
class PartitionedParameterCoordinator:
    ...
    def __init__(
        ...
        self.total_runtime_buffer_time = 0 
    ...
    def fetch_sub_module(self, current_submodule: Module, forward: bool) -> None:
        ...
        params_to_fetch = frozenset(iter_params(current_submodule)) # O
        self.total_runtime_buffer_time += time.time() - start_time

        start_time = time.time()
        fetch_numel = sum(
            [p.partition_numel() for p in params_to_fetch if p.ds_status == ZeroParamStatus.NOT_AVAILABLE]) # O
        self.total_runtime_buffer_time += time.time() - start_time
        ...
        if self.is_complete_trace():
            ...
            params_not_already_fetched = set(
                filter(lambda p: self.__most_recent_step_id_param_fetched_for[p] < self.__step_id, params_to_fetch)) # O
            self.total_runtime_buffer_time += time.time() - start_time

            start_time = time.time()
            while self.__param_queue and len(discarded_from_prefetch_queue) < len(params_not_already_fetched): # O
            ...
            if self.__prefetch_bucket_sz > 0: # O
                start_time = time.time()
                print_rank_0(f"self.__prefetch_bucket_sz: {self.__prefetch_bucket_sz}", force=False)
        ...
        self.__step_id += 1
        print_rank_0(f"self.totla_runtime_buffer_time: {self.total_runtime_buffer_time}", force=True)
