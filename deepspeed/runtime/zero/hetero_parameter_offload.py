...
class DeepSpeedZeRoOffload(object):
    ...
    def _register_hooks_recursively(self, module, prev_module, count=[0]):
        my_count = count[0]
        module.id = my_count

        random_cpu = bool(random.getrandbits(1))
        next_module = module.children()[0]
        ...
        for child in module.children():
            ...
            if not random_cpu:
                ...
                self._register_hooks_recursively(child, prev_module, count=count)

        @instrument_w_nvtx
        def _pre_forward_module_hook(module, *args):
            # asser model params in CPU
            # CPU execution
            # multi-thread로 확인?
            # omp_num_threads
            if random_cpu:
                cpu_partial_out = module(*args)
                # match하지 않고, 
        ...
        def _post_forward_module_hook(module, input, output):
            ...
            # DtoH, CPU
            if random_cpu:
                output[:(len(output)-1)/20].to(, non_blocking=True)
